...
// float *uv=u,*ueps=u+nxyz,*udel=u+2*nxyz;
 ...
 float *fgvepsdel=new float[nn+1];
 float *gvepsdel=fgvepsdel+1;
 ...
 to_header("fgvepsdel","n1",nn+1,"o1",0,"d1",1);
...
//     cout<<"command1 "<<command1[i]<<endl;
 ...
 vector<float> time_in_min; 
 if(!get_array("time_in_min",time_in_min)){
     for(int i=0;i<nserver;i++) time_in_min.push_back(1.f);
 }
...
// vector<int> partition(nserver+1);
 ...
 while((task[0]=='F' && task[1]=='G') || 
  ...
  if(task[0]=='F' && task[1]=='G'){
          fprintf(stderr,"computing objective function and gradient icall=%d",icall);
    ...
    //   }
       ...
       chrono::duration<double> time1=chrono::duration_cast<chrono::duration<double> >(end1-start1);
       cout<<"local server took "<<time1.count()/60.<<" minutes";
       fprintf(stderr,"f_local=%.10f\n",fgcij[0]);
       
       for(int i=0;i<nserver;i++){
         threads[i].join();
         fprintf(stderr,"%s took %f minutes f_server=%.10f\n",server[i].c_str(),time_in_min[i],fgcij_server[i][0]);
       }
       ...
       write("fgvepsdel",fgvepsdel,nn+1,std::ios_base::app);
       to_header("fgvepsdel","n2",icall,"o2",0.,"d2",1.);
...
//   for(int i=0;i<nserver+1;i++) time_in_min[i]=time_in_min[i]/partition[i];
 ...
 delete []vepsdel;delete []cij;delete []fgvepsdel;delete []fgcij;delete []mask;
 delete []m;
